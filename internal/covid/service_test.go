...
func Test_covidServiceImp_GetCovidSummary(t *testing.T) {
	type fields struct {
		Client func(ctrl *gomock.Controller) covid.CovidClient
	}
	tests := []struct {
		name    string
		fields  fields
		want    *covid.CovidResponse
		wantErr bool
	}{
		{
			...
			fields: fields{
				Client: func(ctrl *gomock.Controller) covid.CovidClient {
					...
					b, err := ioutil.ReadFile("./mock/01_covid19_response.json")
					if err != nil {
						t.Fatal(err)
					}

					mockCovid := &covid.Covid19{}
		...
		},
		{
			...
			fields: fields{
				Client: func(ctrl *gomock.Controller) covid.CovidClient {
					...
					b, err := ioutil.ReadFile("./mock/02_covid19_province.json")
					if err != nil {
						t.Fatal(err)
					}
			...
			want:    &covid.CovidResponse{Province: map[string]int{"": 1, "N/A": 1}, AgeGroup: map[string]int{"31-60": 2}},
			wantErr: false,
		},
		{
			...
			fields: fields{
				Client: func(ctrl *gomock.Controller) covid.CovidClient {
					...
					mock.EXPECT().GetCovidCases().Return(&covid.Covid19{}, fmt.Errorf("external error ja"))
